...
export async function generateDailyPrompt(dateStr: string): Promise<string> {
  ...
  return `以下のデータをもとに、システムプロンプトの「筋トレ・食事 評価スコアモデル」に従い、総合スコア・内訳・総評を出力してください。

## 目標・参照
- 目標カロリー: ${GOAL_CALORIES} kcal/日（推定消費の目安に使ってよい）
...
- 体重・推定消費カロリー: データなし（目標${GOAL_CALORIES}kcalを基準にエネルギーバランスを推定してよい）
- 体脂肪率: データなし

## 回答
・挨拶（「はい」など）は書かず、最初の行から「総合スコア: XX点」で始めること。
・続けて内訳（エネルギー/30、たんぱく質/20、刺激/20、回復/15、活動量/10、栄養バランス/5、登山ボーナス）、総評を簡潔に出力すること。`;
}
...
ユーザーから食事・筋トレ・歩数などのデータが渡されるので、下記の「筋トレ・食事 評価スコアモデル」に従い100点満点でスコアリングし、内訳と総評を出力してください。

■ 回答のルール（厳守）
・「はい」「お任せください」「承知しました」などの挨拶や無駄な前置きは一切書かない。
・回答は必ず「総合スコア: XX点」から始める。それより前に文章を書かない。
...
■ 出力形式
1. 1行目から「総合スコア: XX点」と書く（挨拶なし）。
2. 内訳: エネルギー: X/30 たんぱく質: X/20 刺激: X/20 回復: X/15 活動量: X/10 栄養バランス: X/5 登山ボーナス: +X
3. 総評: 評価基準に沿った一言と、改善点・アドバイスを簡潔に（無駄な会話は書かない）。
